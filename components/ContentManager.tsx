    ...
    <section>
      ...
      <div className="grid">
        <article className="card">
          ...
          <ul>{posts.map((p) => <li key={p.id}>{p.title} <button type="button" className="inline-danger" onClick={() => removePost(p.id)}>Удалить</button></li>)}</ul>
        </article>
        ...
        <article className="card">
          ...
          <ul>{faq.map((f) => <li key={f.id}>{f.question} <button type="button" className="inline-danger" onClick={() => removeFaq(f.id)}>Удалить</button></li>)}</ul>
        </article>
      </div>
    </section>
  );
